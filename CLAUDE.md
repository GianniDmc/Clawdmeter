...
- `boards/waveshare_amoled_216_c6/` — Waveshare ESP32-C6-Touch-AMOLED-2.16 (SH8601, 480×480, CST9217 touch). Build env: `waveshare_amoled_216_c6`. ESP32-C6 SoC: single-core RISC-V, **no PSRAM**, BLE 5 only. Rotation is pinned off (`BOARD_ROTATE_WITH_IMU 0` in `board.h`): the panel keeps its boot orientation, while the IMU still feeds the Pomodoro and the edge marks.
